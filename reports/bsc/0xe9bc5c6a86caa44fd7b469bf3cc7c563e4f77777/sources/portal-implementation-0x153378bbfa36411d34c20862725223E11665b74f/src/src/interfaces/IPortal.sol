...
interface IPortalCommonTypes {
    ...
    enum CurveType {
        ...
        CURVE_RH_64000_ASSET, // r = 0.059953125, h = 107036751, k = 66370312.712296875 - 13x curve, ~$64000 ref price, $10K graduation
        CURVE_RH_300_ASSET, // r = 12.79, h = 107036751, k = 14159000045.29 - 13x curve, ~$300 ref price, $10K graduation
        CURVE_RH_450_ASSET, // r = 8.52666667, h = 107036751, k = 9439333367.21678917 - 13x curve, ~$450 ref price, $10K graduation
        CURVE_RH_75_ASSET, // r = 51.16, h = 107036751, k = 56636000181.16 - 13x curve, ~$75 ref price, $10K graduation
        CURVE_RH_10_ASSET, // r = 383.7, h = 107036751, k = 424770001358.7 - 13x curve, ~$10 ref price, $10K graduation
        CURVE_RH_0D07_ASSET, // r = 54814.28571429, h = 107036751, k = 60681428765533.31587179 - 13x curve, ~$0.07 ref price, $10K graduation
        CURVE_RH_45_ASSET, // r = 85.26666667, h = 107036751, k = 94393333638.95678917 - 13x curve, ~$45 ref price, $10K graduation
        CURVE_RH_600_ASSET, // r = 6.395, h = 107036751, k = 7079500022.645 - 13x curve, ~$600 ref price, $10K graduation
        CURVE_RH_125_ASSET, // r = 30.696, h = 107036751, k = 33981600108.696 - 13x curve, ~$125 ref price, $10K graduation
        CURVE_RH_400_ASSET // r = 9.5925, h = 107036751, k = 10619250033.9675 - 13x curve, ~$400 ref price, $10K graduation
    }
...
interface IPortalTypes is IPortalCommonTypes {
    ...
    ///      token with no cap set never emits this event. Both fields are snapshots of the origin's
    ///      live balance taken after the buy, matching buyQuotaOf: the cap is a holding cap, so they
    ///      also change on transfers and sells, which emit no event. Reconstruct those from ERC-20
    ///      Transfer logs (or read buyQuotaOf) rather than treating this event as the full history.
    /// @param token The address of the token
    /// @param origin The tx.origin address whose quota usage changed
    /// @param bought The origin's token balance after the buy, i.e. what counts against the cap
...
interface IPortalLensV2 is IPortalTypes {
    ...
    /// @notice Get the per-tx.origin buy cap for a token on the internal market.
    /// @dev token-level config, independent of any origin. The cap is enforced as a HOLDING cap:
    ///      an origin's allowance for a curve buy is `maxBuyAmount - balanceOf(tx.origin)`, so it
    ///      is restored when the origin's balance drops and consumed when tokens move in. Use
    ///      buyQuotaOf(token, origin) for a specific origin's live allowance.
    /// @param token The address of the token
    ...
    /// @notice Get an origin's per-token buy quota on the internal market: how much currently counts
    ///         against the cap, and how much it can still buy right now.
    /// @dev The cap is a HOLDING cap and BOTH values are derived from the origin's live balance. No
    ///      cumulative purchase counter is kept: `bought` is balanceOf(origin), NOT a lifetime total,
    ///      and `remaining` is `cap - balanceOf(origin)` floored at 0.
    ///
    ///      So the allowance grows back whenever the origin's balance drops (selling to the curve OR
    ///      transferring out) and shrinks when tokens move in (including transfers and airdrops the
    ///      origin never bought). The cap applies to every buy including a token creator's launch buy
    ///      — there is no launch-buy exemption. Buys beyond the allowance are refunded, not reverted.
    ///
    ///      Two consequences are known and accepted: (1) buy → transfer out → buy can be repeated
    ///      from one origin (even inside a single transaction) to consume the curve up to the DEX
    ///      threshold; (2) curve buys credit msg.sender, so when a contract buys and holds the
    ///      tokens the origin EOA's balance stays 0 and its allowance is never consumed.
    /// @param token The address of the token
    /// @param origin The tx.origin address to query
    /// @return bought The origin's current token balance, i.e. what counts against the cap
    /// @return remaining The live buy allowance in token units (0 when the origin already holds the
    ///         cap; type(uint256).max if the token has no cap, i.e. maxBuyPerOrigin's bps == 0)
...
interface IPortalTweak is IPortalTypes {
    ...
    /// @dev Only callable by the default admin, TAX_MANAGER_ROLE, or MANAGE_ROLE
    ...
    /// @dev Only callable by MANAGE_ROLE or DEFAULT_ADMIN_ROLE. The excluded address must be a
    ///      non-EOA contract and must not be an EIP-7702 delegated EOA.
    ...
    /// @dev Restricted to MANAGE_ROLE or DEFAULT_ADMIN_ROLE.
    ...
    /// @dev Only callable by MANAGE_ROLE or DEFAULT_ADMIN_ROLE.
    ...
    /// @notice Add or remove an origin from a V2/V3 tax token's Dividend deferral set.
    /// @dev Only callable by MANAGE_ROLE or DEFAULT_ADMIN_ROLE. Resolves the Dividend contract via
    ///      `dividendContract()` and forwards to `Dividend.setDeferredOrigin`.
    /// @param taxToken The tax token address.
    /// @param origin The tx.origin to add or remove from the deferral set.
    /// @param deferred True to defer first-receive registration for this origin.
    function setTaxTokenDeferredOrigin(address taxToken, address origin, bool deferred) external;

    /// @notice Add or remove an address from a V2/V3 tax token's Dividend `refreshShares` keeper set.
    /// @dev Only callable by MANAGE_ROLE or DEFAULT_ADMIN_ROLE. Resolves the Dividend contract via
    ///      `dividendContract()` and forwards to `Dividend.setKeeper`.
    /// @param taxToken The tax token address.
    /// @param keeper The address to add or remove as a keeper.
    /// @param allowed True to grant keeper status.
    function setTaxTokenDividendKeeper(address taxToken, address keeper, bool allowed) external;

    /// @notice Redirect a holder's dividend payouts to another address on a V2/V3 tax token's
    ///         Dividend contract.
    /// @dev Only callable by MANAGE_ROLE or DEFAULT_ADMIN_ROLE. Resolves the Dividend contract via
    ///      `dividendContract()` and forwards to `Dividend.setDividendReceiver`.
    /// @param taxToken The tax token address.
    /// @param holder The holder whose dividends are redirected.
    /// @param receiver Destination, or address(0) to clear the redirect.
    function setTaxTokenDividendReceiver(address taxToken, address holder, address receiver) external;
    ...
    ///         targeting a ~$50 dispatch / ~$25 buyback floor instead of the value-blind
    ///         `20/50 * 10^decimals` init default.
