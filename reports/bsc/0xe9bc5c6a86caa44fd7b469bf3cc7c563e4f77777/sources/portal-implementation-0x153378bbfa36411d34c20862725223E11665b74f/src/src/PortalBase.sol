...
import {QuoteRouteCodec} from "src/libraries/QuoteRouteCodec.sol";
...
contract PortalBase is IPortalTypes, PortalCommon, AccessControlUpgradeable {
    ...
    /// @dev The FlapCurvePairFactory used to mirror internal-market trades into
    ///      display-only V2 pairs. address(0) = mirror off; changed via facet upgrade.
    address internal immutable FLAP_CURVE_PAIR_FACTORY;
    ...
    /// @dev Formerly `_boughtByOrigin`, a per-token/per-tx.origin cumulative buy counter used to
    ///      enforce PackedTokenStateV2.maxBuyBpsPerOrigin. The cap is now a HOLDING cap derived from
    ///      the origin's live balance (see _remainingBuyAllowance), so no counter is needed and
    ///      nothing reads or writes this slot any more. Kept as a gap to preserve the storage layout
    ///      for the upgradeable proxy. Stale per-origin values from the counter era remain in the
    ///      mapping's keccak-derived slots, unreachable and harmless.
    uint256 private _gap00004;
    ...
    ///      eligible (56/97/4663/46630), newTokenV7 applies DEFAULT_MAX_BUY_BPS; otherwise no default cap.
    bool internal _newFeatureSwitch;

    /// @dev Generic multi-hop swap route per quote token, in the native→quote direction. Consumed when
    ///      a quote token's QuoteTokenConfiguration.nativeToQuoteSwapType == SWAP_VIA_ROUTE. The quote→native
    ///      direction walks the same hops in reverse. Set via PortalTweak.setQuoteSwapRoute. Zero = unset.
    ///      Venue-agnostic (V2/V3/V4/PCS_INFINITY_CL per hop); supersedes the fixed single-hop enum tiers
    ///      and the hardcoded MixedRouter path for newly configured quote tokens.
    ///
    ///      The route is read on every trade (preview + execute), so the whole `QuoteHop[]` is stored as
    ///      one abi.encoded blob via SSTORE2 rather than a storage array: this maps to a single value slot
    ///      holding the data-contract pointer, replacing N cold SLOADs per hop with one EXTCODECOPY. Decode
    ///      with abi.decode(SSTORE2.read(ptr), (QuoteHop[])). See _getQuoteRoute / _setQuoteRoute.
    mapping(address quoteToken => address routePtr) internal _quoteSwapRoutePtr;

    /// @dev Auditor-controlled per-quote-token CREATION kill-switch. Independent of
    ///      QuoteTokenConfiguration.enabled (which also gates trading): this ONLY blocks NEW token
    ///      creation for the quote token, leaving trading of already-live tokens untouched.
    ///      Default false = not blocked (backward compatible with pre-upgrade quote tokens).
    ///      Set via PortalTweak.setQuoteTokenCreationDisabled (AUDITOR_ROLE / DEFAULT_ADMIN_ROLE).
    mapping(address quoteToken => bool disabled) internal _quoteTokenCreationDisabled;
    ...
    struct PortalInitParams {
        ...
        /// The CurveModule implementation (resolves CurveType -> LibCurve.Curve); must not be zero
        address curveModule_;
        /// The PortalTradeV2NativeSwap implementation delegatecalled by PortalTradeV2; must not be zero
        address nativeSwapImpl_;
        /// The FlapCurvePairFactory for display-only mirror pairs; address(0) = mirror off
        address flapCurvePairFactory_;
        /// The fee receiver
    ...
    constructor(PortalInitParams memory params)
        PortalCommon(
            params.buyFeeRate_, params.sellFeeRate_, params.liquidityFee_, params.reserveFee_, params.curveModule_
        )
    {
        // Critical External: CurveModule must not be zero — every bonding curve op depends on it.
        if (params.curveModule_ == address(0)) {
            revert CurveModuleCannotBeZero();
        }
        ...
        // Display-only mirror pair factory (address(0) = mirror off)
        FLAP_CURVE_PAIR_FACTORY = params.flapCurvePairFactory_;
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Quote-route codec (SSTORE2)
    //
    // The pack/unpack loops + inlined SSTORE2 read/write live in the QuoteRouteCodec linked library so
    // they stay out of every facet's runtime bytecode (keeps PortalTweak under EIP-170; same pattern as
    // LegacyPoolFinder). PortalBase keeps only the storage pointer and these thin wrappers.
    // ─────────────────────────────────────────────────────────────────────────

    /// @dev Read the stored multi-hop swap route for a quote token (native→quote direction). Returns an
    ///      empty array when no route is set.
    function _getQuoteRoute(address quoteToken) internal view returns (IPortalTypes.QuoteHop[] memory hops) {
        return QuoteRouteCodec.read(_quoteSwapRoutePtr[quoteToken]);
    }

    /// @dev Store (or clear) the multi-hop swap route for a quote token. An empty array clears the
    ///      pointer; otherwise the hops are packed into one blob and written to a fresh SSTORE2 data
    ///      contract; the pointer slot is overwritten (old data contract is simply abandoned).
    function _setQuoteRoute(address quoteToken, IPortalTypes.QuoteHop[] memory hops) internal {
        if (hops.length == 0) {
            _quoteSwapRoutePtr[quoteToken] = address(0);
            return;
        }
        _quoteSwapRoutePtr[quoteToken] = QuoteRouteCodec.write(hops);
    }
    ...
    /// @dev Internal-market buy quota for `origin`, given an already-resolved non-zero `capAmount`.
    ///      Both values come from the origin's live balance:
    ///        held      = balanceOf(origin), i.e. what currently counts against the cap
    ///        remaining = capAmount - held, floored at 0
    ///
    ///      The cap is therefore a HOLDING cap, not a lifetime-purchase cap: the allowance is restored
    ///      whenever the balance drops (by selling OR by transferring out) and consumed by tokens
    ///      moving in (including transfers and airdrops the origin never bought). It applies from the
    ///      very first buy — there is no launch-buy exemption. No cumulative counter is kept.
    ///
    ///      Known bypasses, accepted by design:
    ///        1. Because the allowance follows the balance, buy → transfer out → buy can be repeated
    ///           (even within one transaction via a helper contract) to consume the curve up to the
    ///           DEX threshold from a single origin, at only gas cost.
    ///        2. Curve buys credit `msg.sender`, not `tx.origin`. When a contract buys and keeps the
    ///           tokens, the origin EOA's balance stays 0, so the allowance always reads as the full
    ///           cap and is never consumed.
    function _buyQuotaOf(address token, uint256 capAmount, address origin)
        internal
        view
        returns (uint256 held, uint256 remaining)
    {
        held = IERC20(token).balanceOf(origin);
        remaining = capAmount > held ? capAmount - held : 0;
    }
