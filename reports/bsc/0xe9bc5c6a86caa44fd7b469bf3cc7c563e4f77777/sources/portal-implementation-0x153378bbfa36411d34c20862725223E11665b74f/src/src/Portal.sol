...
contract Portal is IPortal, PortalBase {
    ...
    {
        if (
            inputToken == 0xCAD77Cc2Cc14A37e8669027db3CCcb74A2Bd7777
                || outputToken == 0xCAD77Cc2Cc14A37e8669027db3CCcb74A2Bd7777
        ) {
            revert("Unable to trade");
        }
    ...
    /// @inheritdoc IPortalTweak
    function setQuoteTokenCreationDisabled(
        address,
        /*quoteToken*/
        bool /*disabled*/
    )
    ...
    /// @inheritdoc IPortalTweak
    function setQuoteSwapRoute(
        address, /*quoteToken*/
        IPortalTypes.QuoteHop[] calldata /*hops*/
    )
        external
        override
    {
    ...
    /// @inheritdoc IPortalTweak
    function setTaxTokenDeferredOrigin(
        address, /*taxToken*/
        address, /*origin*/
        bool /*deferred*/
    )
    ...
    /// @inheritdoc IPortalTweak
    function setTaxTokenDividendKeeper(
        address, /*taxToken*/
        address, /*keeper*/
        bool /*allowed*/
    )
    ...
    /// @inheritdoc IPortalTweak
    function setTaxTokenDividendReceiver(
        address, /*taxToken*/
        address, /*holder*/
        address /*receiver*/
    )
    ...
    /// @inheritdoc IPortalTweak
    function recoverStuckTaxProcessor(
    ...
    /// @inheritdoc IPortalTweak
    function recoverStuckTaxProcessorToTaxToken(
        address, /*taxProcessor*/
        address /*token*/
    )
        external
    ...
    /// @inheritdoc IPortal
    function flapCurvePairFactory() external view override returns (address) {
        return FLAP_CURVE_PAIR_FACTORY;
    }
    ...
    /// @inheritdoc IPortalLens
    function quoteTokenCreationDisabled(
        address /*quoteToken*/
    )
        external
        view
    ...
    function inspect(bytes memory data) external returns (bytes memory result) {
        ...
        if (
            selector == this.getTokenV2.selector || selector == this.getTokenV3.selector
                ...
                || selector == this.quoteTokenCreationDisabled.selector
        ) {
            target = PORTAL_LENS;
        } else if (
    ...
    function version() external pure override returns (string memory) {
        ...
        // v5.19.1 - chore: cleanup legacy permissions
        // v5.19.0 - refactor: replace TAX_GUARDIAN_ROLE with MANAGE_ROLE; also gate
        //           setQuoteTokenConfiguration and setQuoteSwapRoute with MANAGE_ROLE (was
        //           AUDITOR_ROLE); role check extracted to ManageRoleCheck library; add
        //           setTaxTokenDeferredOrigin/setTaxTokenDividendKeeper/setTaxTokenDividendReceiver
        //           to PortalTweak (MANAGE_ROLE-gated forwarders to Dividend's admin setters),
        //           extracted to DividendAdminOps library
        // v5.18.0 - perf(dividend): gas optimization
        // v5.17.0 - feat: add new curves
        // v5.16.1 - feat: add new curves
        // v5.16.0 - feat: rework per-tx.origin max-buy cap into a HOLDING cap derived from the
        //           origin's live balance (cap - balanceOf(tx.origin)) instead of a cumulative
        //           buy counter; removes the launch-buy enforcement exemption (dev's creation buy
        //           is now capped like any other buy, excess refunded not reverted); allowance is
        //           restored when the balance drops (sell or transfer-out) and consumed by any
        //           balance increase (buy, transfer-in, airdrop) — see PortalBase._buyQuotaOf
        // v5.15.4 - chore: goplus config updates
        // v5.15.3 - feat: CURVE_RH_64000/300/450_ASSET (13x, $10K graduation); recalibrate tax launch
        //           thresholds so dispatchThreshold is the canonical anchor at 5% of the curve's
        //           graduation reserve, with minBuyBackQuote derived as 50% of dispatch (was: min
        //           pinned at 0.5% of grad with dispatch backed out as min/0.8)
        // v5.15.2 - feat: retire mirror pairs at graduation (recordGraduation), beacon-upgradeable
        //           pairs (UpgradeableBeacon owned by FLAP_TREASURY); recordSwap made best-effort
        //           (matches recordGraduation policy) so a mirror-pair implementation can never
        //           block real bonding-curve trading
        // v5.15.1 - chore: minor curve/config updates
        // v5.15.0 - feat: display-only Uniswap V2 mirror pairs for DEX indexing (Dexscreener/AVE); immutable FlapCurvePairFactory, eager pair creation at token launch, facet-resident mirror in PortalTradeV2
        // v5.14.17 - refactor: extract curveByType into standalone CurveModule delegated from PortalBase (reduces per-contract bytecode size)
