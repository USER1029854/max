...
Raw evidence (decoded live-state JSON, proxy/admin resolution, Safe signer data, full ABI dumps) is included under [`evidence/`](./evidence/) alongside this report. The **complete, unmodified verified source** for all 19 resolved contracts in the graph (target implementation, every downstream and upstream contract, both Gnosis Safe proxies and their actual singleton logic contracts) is under [`sources/`](./sources/) — see [`sources/INDEX.md`](./sources/INDEX.md) for the full directory-to-address map, including the handful of addresses where no source exists at all (and why).
